...
scene("fight", () => {
    ...
    */
    ...
    // Automate CPU behavior
    loop(1, () => {
        if (gameOver) return;

        // CPU attacks Player 1 every second
        attack(player2, []);
    });
    ...
    player2.onUpdate(() => {
        ...
        // CPU moves towards Player 1
        if (player2.pos.x > player1.pos.x) {
            run(player2, -200, true); // Move left
        } else {
            run(player2, 200, false); // Move right
        }
    });

    // Ensure CPU always wins
    ...
    // Disable Player 2's health reduction
